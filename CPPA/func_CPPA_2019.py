...
#    n_time = times.size
    options = [f'{int(lag)}_100_CPPAspatcov', f'{int(lag)}_101_PEPspatcov']
...
#            for col in options:
            if select == 'sst':
                col = f'{int(lag)}_100_CPPAspatcov'
                mask_name = 'prec_labels'
            elif select == 'PEP':
                col = f'{int(lag)}_101_PEPspatcov'
                mask_name = 'PEP_mask'

            lag = int(col.split('_')[0])
...
#    return out
#
#def prevent_lag_mismatch(dates_lag, dates_available):
#    if dates_lag.min() < dates_available.min():
#        mask_matching = dates_lag - dates_available.min()> pd.Timedelta(0,unit='d')
