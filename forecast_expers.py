...
d_t_l = get_freqs_same_lag(LAG_DAY)   

#d_t_l = {f:1 for f in range(27,35)}
#d_t_l = {f:1 for f in range(8,16)}
d_t_l = {f:1 for f in range(3,11)}
frequencies = list(d_t_l.keys())
min_lag = frequencies[-1] 
#min_lag = 10
fold = 0
method='ran_strat9'
seed=30
...
#        if tfreq == 1:    
#            lags_i = np.array([max(1, int(min_lag/(tfreq)))])
#        else:
#            lags_i = np.array([max(1, int(min_lag/(0.5*tfreq)))])
...
f_name = f'{RV_name}_lag_{LAG_DAY}d_{percentile}p_{today}'
...
for f_format in f_formats:
    ...
    fig = dfplots.plot_score_expers(d_expers=dict_experiments, model=stat_model_l[-1][0], 
                      metric=metric, lags_t=np.array(lags_t,dtype=int),
